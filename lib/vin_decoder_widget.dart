    ...
    try {
      ...
      setState(() {
        decodingResult = '''
          ...
          Safety Features: ${decoder.safetyFeatures}
        ''';
      });
    } catch (e) {
      setState(() {
  ...
  void decodeWmiCode() {
    ...
    if (wmiDetails.containsKey(wmiCode)) {
      setState(() {
        decodingResult += '''
          \n\nWMI Code: $wmiCode
          WMI Details: ${wmiDetails[wmiCode]!}
        ''';
      });
    } else {
      setState(() {
...
class VinDecoder {
  ...
  Future<void> decodeVehicleDetails(String vin) async {
    ...
    if (!(data['Message']?.contains("Results returned successfully") ??
        false)) {
      throw Exception("VIN decoder service error: ${data['Message']}");
    }
    ...
    for (var result in data) {
      ...
      switch (name) {
        ...
        case "Engine Model":
          engineType = value;
          break;
        case "Safety Restraint System":
          safetyFeatures = value;
          break;
        case "Error Code":
          if (value != "0") {
            return;
